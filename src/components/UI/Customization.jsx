...
const Customization = () => {
  const carColors = [
    ...
    "#554FA8", // Dark Slate Purple
    "#178F86", // Muted Teal
    "#C9A227", // Dark Warm Gold
    "#D45A96", // Muted Rose Pink
    "#6F3A17", // Deep Saddle Brown
    "#246B47", // Dark Sea Green
    "#9E9E9E", // Dark Silver Metallic
      ...
      <div className="pt-4 ">
        ...
        <div className="grid grid-cols-6 md:grid-cols-6 gap-4">
          {carColors.map((color, idx) => (
            <span
              className={`w-6 h-6 md:w-10 md:h-10 rounded-full cursor-pointer inline-block ${currentColor === color ? "ring-4 ring-offset-2 ring-blue-500" : ""}`}
              key={idx}
